// SPDX-Copyright-Text: \169 2023 CHiPs44 <chips44@gmail.com>
...
*HAGL* is a generic library for bitmap graphics, originally written with I²C or SPI LCD panels in mind.
...
* Multiplane & VT100/ANSI terminal are on the way…

=== Pico stuff

==== *pshell* by *lurk101*

*pshell* is a serial accessible shell with C compiler & editor.

* base commands: `ls`, `cd`, `mkdir`, `cat`, …
* `vi` editor (from an old busybox version)
* `cc`, C compiler generating ARM M0+ for RP2040 and now M33 code for RP2350
...
*pt52-lua* is Lua embedded on PT52 (a re-imagining of the VT52 terminal by Digital).
